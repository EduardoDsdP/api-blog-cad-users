# Novo arquivo de testes unitarios:

from flask import Flask, jsonify, request, make_response
from estrutura_banco_dados import Autor, Postagem, app, db
import json
import jwt   # import para criar o token de validaçãao para autenticação de usuario
...
# esse codigo abaixo pode ser aplicado áa qualquer rota que precisa ser segura e precise de autenticação. 
# e colocando um @token_obrigatorio em tods as funções, estamos apontando que todos os processos precisam de auteenticação do token. 
def token_obrigatorio(f):
    @wraps(f)
    def decorated(*args, **kwargs):  # nesse decoretor eesta sendo passado aargumentos posicionais e argumentos relacionados.  
        token = None
...
# Obter postagem por id - GET https://localhost:8000/postagem/1
...
# Criar uma nova postagem - POST https://localhost:8000/postagem
...
# Alterar uma postagem existente - PUT https://localhost:8000/postagem/1
...
# Excluir uma postagem - DELETE - https://localhost:8000/postagem/1
...
def alterar_autor(autor,id_autor):
    ...
    autor = Autor.query.filter_by(id_autor = id_autor).first()
    if not autor:
        return jsonify({'Mensagem': 'usuario não foi encontrado'})
    try:
...
def excluir_autor(id_autor):
    ...
    if not autor_existente:
        return jsonify({'mensagem': 'este usuario autor não foi encontrado.'})
